    ...
    <Modal open={isOpen} onClose={onClose} title="Execute Swap Order">
      <div className="space-y-4">
        <p className="text-text-secondary">
          You are about to {isBuy ? "buy" : "sell"} <span className="font-bold text-text-primary">{order.amount} {order.tokenIn}</span> for <span className="font-bold text-text-primary">{order.total} {order.tokenOut}</span>.
        </p>
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="text-right font-mono text-text-primary">{order.amount} {order.tokenIn}</div>

          <div className="text-text-muted">Price:</div>
          <div className="text-right font-mono text-text-primary">{order.price} {order.tokenOut}/{order.tokenIn}</div>

          <div className="text-text-muted">Total:</div>
          <div className="text-right font-mono text-text-primary">{order.total} {order.tokenOut}</div>
...
export function OrderBookList({ orders, onTakeOrder, userAddress }: OrderBookListProps) {
  ...
  const filteredOrders = useMemo(() => {
    return orders
      .filter((o) => {
        const matchesSearch = o.pair.toLowerCase().includes(search.toLowerCase()) || 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-overlay/50 border-b border-border">
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-text-muted cursor-pointer hover:text-text-primary" onClick={() => handleSort("pair")}>
                  <div className="flex items-center gap-2">Pair <ArrowUpDown size={12} /></div>
                </th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-text-muted cursor-pointer hover:text-text-primary" onClick={() => handleSort("side")}>
                   <div className="flex items-center gap-2">Side <ArrowUpDown size={12} /></div>
                </th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-text-muted cursor-pointer hover:text-text-primary" onClick={() => handleSort("amount")}>
                   <div className="flex items-center gap-2">Amount <ArrowUpDown size={12} /></div>
                </th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-text-muted cursor-pointer hover:text-text-primary" onClick={() => handleSort("price")}>
                   <div className="flex items-center gap-2">Price <ArrowUpDown size={12} /></div>
                </th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-text-muted">Total</th>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-text-secondary font-medium">No active orders matching filters.</p>
                      <Button variant="ghost" size="sm" onClick={() => { setSearch(""); setSideFilter("all"); }}>
                        Clear Filters
                      </Button>
                    </div>
                  </td>
